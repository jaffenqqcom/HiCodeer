...
/// OHOS: the binary must land on the VM (the device sandbox is not visible
/// there), so the whole download runs as VM commands via `util::command`
/// (which routes every command through the cmd-agent): curl fetches,
/// sha256sum verifies, tar/gunzip extracts, mv finalizes. Paths are device
/// paths the server maps to their VM equivalents, staying under the same LSP
/// root (`$HOME/zed/languages/<server>`) as npm-installed servers.
#[cfg(target_env = "ohos")]
pub async fn download_server_binary(
    ...
    log::info!("downloading github artifact from {url} (on VM)");
    let Some(destination_parent) = destination_path.parent() else {
        anyhow::bail!("destination path has no parent: {destination_path:?}");
    };

    // A staging directory next to the destination; on the VM (after path
    // mapping) it is `<root>/zed/languages/<server>/.tmp-zcoder-download`.
    let staging_dir = destination_parent.join(".tmp-zcoder-download");
    // The Gz case downloads a gzipped raw binary; name the file with a `.gz`
    // suffix so `gunzip -k` can strip it.
    let staging_file_name = match asset_kind {
        AssetKind::Gz => "archive.dl.gz",
        _ => "archive.dl",
    };
    let staging_file = staging_dir.join(staging_file_name);

    util::command::new_command("mkdir")
        .arg("-p")
        .arg(&staging_dir)
        .output()
        .await
        .context("creating download staging dir on the VM")?;

    let curl = util::command::new_command("curl")
        .args(["-L", "-sS"])
        .arg("-o")
        .arg(&staging_file)
        .arg(url)
    ...
    if let Some(expected_sha_256) = digest {
        let sum = util::command::new_command("sha256sum")
            .arg(&staging_file)
            .output()
            .await
            .context("sha256sum failed to start on the VM")?;
        let actual = String::from_utf8_lossy(&sum.stdout)
            .split_whitespace()
            .next()
            .unwrap_or_default()
            .to_string();
        anyhow::ensure!(
            sha256_matches(&actual, expected_sha_256),
            "{url} asset got SHA-256 mismatch. Expected: {expected_sha_256}, Got: {actual}"
        );
    }

    // Gz assets are a gzipped raw binary: decompress and move the result to
    // the destination (a single file). Archive assets extract into the staging
    // dir, which is then moved to the destination (a directory).
    if asset_kind == AssetKind::Gz {
        util::command::new_command("gunzip")
            .arg("-k")
            .arg(&staging_file)
            .output()
            .await
            .context("gunzip failed on the VM")?;
        let decompressed = staging_dir.join("archive.dl");
        util::command::new_command("rm")
            .arg("-f")
            .arg(&staging_file)
            .output()
            .await
        ...
        util::command::new_command("rm")
            ...
            .await
            .ok();
        util::command::new_command("mv")
            .arg(&decompressed)
            .arg(destination_path)
            .output()
            .await
            .context("mv decompressed binary failed on the VM")?;
        util::command::new_command("rm")
            .arg("-rf")
            .arg(&staging_dir)
            .output()
    ...
    match asset_kind {
        AssetKind::TarGz => {
            util::command::new_command("tar")
                ...
                .output()
                .await
                .context("tar extract failed on the VM")?;
        }
        AssetKind::TarBz2 => {
            util::command::new_command("tar")
                .arg("-xjf")
        ...
        AssetKind::Gz => unreachable!("handled above"),
        AssetKind::Zip => {
            anyhow::bail!(
                "zip asset extraction is unsupported on OHOS (the VM lacks unzip/bsdtar): {url}"
            );
        }
    }
    ...
    util::command::new_command("mv")
        ...
        .output()
        .await
        .context("mv extracted artifact failed on the VM")?;
...
/// OHOS variant of `download_server_raw_binary`; same rationale as the OHOS
/// `download_server_binary` — the file is fetched with curl on the VM so it
/// lands where commands run.
#[cfg(target_env = "ohos")]
pub async fn download_server_raw_binary(
    _http_client: &dyn HttpClient,
    url: &str,
    ...
    log::info!("downloading raw binary from {url} (on VM)");
    let Some(destination_parent) = destination_path.parent() else {
        anyhow::bail!("destination path has no parent: {destination_path:?}");
    };

    let staging_dir = destination_parent.join(".tmp-zcoder-download");
    let binary_path = staging_dir.join(binary_file_name);
    ...
    util::command::new_command("mkdir")
        ...
        .output()
        .await
        .context("creating staging dir on the VM")?;
    let curl = util::command::new_command("curl")
        .args(["-L", "-sS"])
        .arg("-o")
        .arg(&binary_path)
    ...
    // Executable bit is applied by the caller via `make_file_executable`,
    // which routes to the VM on OHOS.
    util::command::new_command("rm")
        .arg("-rf")
        .arg(destination_path)
        .output()
    ...
    util::command::new_command("mv")
        ...
        .output()
        .await
        .context("mv binary failed on the VM")?;
    Ok(())
}

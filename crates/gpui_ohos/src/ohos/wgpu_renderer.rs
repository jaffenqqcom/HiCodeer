...
impl WgpuRenderer {
    ...
    pub fn new<W: HasWindowHandle + HasDisplayHandle>(
        ...
        log::info!(
            "[boot] WgpuRenderer::new entered, config size {}x{}",
            config.size.width.0,
        ...
        log::info!("[boot] WgpuRenderer: surface created");
        ...
        log::info!(
            "[boot] WgpuRenderer: surface caps formats={:?}, alpha_modes={:?}; chosen format={:?}, alpha_mode={:?}",
        ...
        log::info!(
            "[boot] WgpuRenderer: surface configured, {}x{}, present_mode={:?}",
        ...
        log::info!(
            "[boot] WgpuRenderer initialized successfully, adapter {:?}",
            adapter_info.name
        );
    ...
    pub fn draw(&mut self, scene: &Scene) {
        ...
        let frame = match self.surface.get_current_texture() {
            ...
            wgpu::CurrentSurfaceTexture::Suboptimal(frame) => {
                log::warn!("[boot] draw: surface Suboptimal, reconfiguring");
                drop(frame);
                self.surface.configure(&self.device, &self.surface_config);
                return;
            }
            wgpu::CurrentSurfaceTexture::Lost | wgpu::CurrentSurfaceTexture::Outdated => {
                log::warn!("[boot] draw: surface Lost/Outdated, reconfiguring");
                self.surface.configure(&self.device, &self.surface_config);
                return;
            }
            wgpu::CurrentSurfaceTexture::Timeout | wgpu::CurrentSurfaceTexture::Occluded => {
                log::warn!("[boot] draw: surface Timeout/Occluded, skipping frame");
                return;
            }
            wgpu::CurrentSurfaceTexture::Validation => {
                log::warn!("[boot] draw: surface Validation error, skipping frame");
                return;
            }
        };
    ...
    fn draw_instances(
        ...
        pass.set_vertex_buffer(0, self.instance_buffer.slice(offset..offset + size.get()));
    ...
    fn draw_instances_with_texture(
        ...
        let bind_group = self.device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: None,
            layout: &self.bind_group_layouts.textures,
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,

...
fn uri_to_local_path(uri: &str) -> Option<PathBuf> {
    ...
    if err != 0 || result.is_null() {
        log::error!(
            "ohos_file_geturi::uri_to_local_path: OH_FileUri_GetPathFromUri failed err={err}"
        );
        return None;
    }
    let path = unsafe {
...
pub fn ensure_root_authorized(path: &str) -> PathBuf {
    // [diag] trace the OHOS worktree-root authorization so a silent failure
    // (URI conversion or permission activation) is visible in the log.
    log::info!("[diag] ensure_root_authorized: path={path}");
    if !path.starts_with(USER_PUBLIC_PATH_PREFIX) {
        return PathBuf::from(path);
    }
    let Some(uri) = path_to_uri(path) else {
        log::warn!("[diag] ensure_root_authorized: path_to_uri failed for {path}");
        return PathBuf::from(path);
    };
    log::info!("[diag] ensure_root_authorized: uri={uri}");
    if !activate_permission(&uri) {
        log::warn!("[diag] ensure_root_authorized: activate_permission failed for {path}");
        return PathBuf::from(path);
    }
    // [diag] surface the authorized local path; a sandbox path here means the
    // worktree scans an authorized view, an unchanged path means fallback.
    log::info!(
        "[diag] ensure_root_authorized: local_path={:?}",
        uri_to_local_path(&uri)
    );
...
fn path_to_uri(path: &str) -> Option<String> {
    ...
    if err != 0 || result.is_null() {
        log::error!(
            "ohos_file_geturi::path_to_uri: OH_FileUri_GetUriFromPath failed err={err} path={path}"
        );
        return None;
    }
    let uri = unsafe {
...
fn call_permission_api(uri: &str, activate: bool) -> bool {
    let c_uri = match CString::new(uri) {
        Ok(value) => value,
        Err(error) => {
            log::error!("ohos_file_geturi: invalid URI bytes: {error}");
            return false;
        }
    };
    ...
    if result != 0 {
        log::error!("ohos_file_geturi::{api_name} failed: err={result} uri={uri}");
        return false;
    }

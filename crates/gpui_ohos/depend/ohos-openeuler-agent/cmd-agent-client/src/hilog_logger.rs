...
mod ohos {
    ...
    impl Log for HilogLogger {
        ...
        fn log(&self, record: &Record) {
            // Prefix the record target (crate/module path) so logs from the
            // daemon, client, and deploy modules stay distinguishable in hilog.
            let message = format!("[cmd-agent:{}] {}", record.target(), record.args());

//! Embedded russh SSH server.
//!
//! Accepts SSH connections from the zcoder host over the slirp network,
//! authenticates with the client public key (password auth disabled), and runs
//! exec commands inside the guest. Channel stdin is forwarded to the running
//! child; stdout/stderr and the exit status are bridged back by `exec`.
...
pub struct ConnectionHandler {
    ...
    conn_id: u64,
    /// Accepted client public keys (the one-time keypair generated at boot).
...
impl server::Handler for ConnectionHandler {
    ...
    async fn data(
        ...
        log::info!(
            "[diag] guest ssh: conn {} data {} bytes on channel {channel}",
            self.conn_id,
            data.len()
        );
    ...
    async fn channel_eof(
        ...
        // output (compute_snapshot's revparse_batch) hangs indefinitely.
        log::info!(

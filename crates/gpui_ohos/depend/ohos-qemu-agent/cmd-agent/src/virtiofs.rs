...
/// Starts the virtio-fs backends for the static sandbox/tools mounts. Each
/// backend runs on its own thread and listens on its socket; QEMU's
/// vhost-user-fs-pci chardev connects to it when the guest machine starts.
...
/// Each work directory gets its own backend (and listening socket); QEMU's
/// chardev connects to it and the guest mounts the tag. Returns the socket
/// path for the QMP chardev-add.
...
fn run_backend(tag: &str, socket_path: &Path, shared_dir: &Path, readonly: bool) -> Result<()> {
    // PassthroughFsRo and PassthroughFs are distinct types, so branch before
    // the generic serve() rather than returning one or the other.
    ...
    F: FileSystem + SerializableFileSystem + Send + Sync + 'static,
{
    ...
    // Listener::new(path, true) created a listening socket; QEMU's chardev
    // connects to it as the vhost-user client.

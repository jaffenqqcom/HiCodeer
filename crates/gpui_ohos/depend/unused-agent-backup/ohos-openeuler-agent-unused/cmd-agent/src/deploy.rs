...
pub fn deploy(cfg: &SshConfig, server_binary: &[u8], listen_port: u16) -> Result<()> {
    ...
    rt.block_on(async move {
        let remote_path = format!("{}/cmd-agentd", cfg.remote_dir);
...
/// Kills any stale cmd-agentd process by exact name, so the fresh
...
async fn kill_old_server(session: &mut SshSession) -> Result<()> {
    let command = "pkill -x cmd-agentd; true".to_string();

...
fn open_user(controller: &OwnedFd, flags: rustix::pty::OpenptFlags) -> io::Result<OwnedFd> {
    ...
    // try to optimize. Skip it on OpenHarmony too (target_env="ohos"): its
    // sandbox rejects the ioctl with EACCES, which is not among the tolerated
    // fallback errnos (EPERM/ENOSYS), so it would abort openpty here. Falling
    // through to `ptsname` + `openat` matches glibc semantics.

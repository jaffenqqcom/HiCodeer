...
pub(crate) fn open_impl(
    ...
    // On OpenHarmony (`target_env = "ohos"`) `openat2` (syscall 437) is not in
    // the application seccomp allow-list, so invoking it terminates the process
    // with `SIGSYS` instead of returning `ENOSYS`. Skip `openat2` entirely and
    // fall through to `manually::open`, which enforces `BENEATH` semantics in
    // userspace via `openat` (which is allowed).
    #[cfg(all(target_os = "linux", not(target_env = "ohos")))]
    {
...
// On OpenHarmony (`target_env = "ohos"`) `openat2` (syscall 437) is not in the
// application seccomp allow-list, so invoking it terminates the process with
// `SIGSYS` instead of returning `ENOSYS`. Provide an `open_beneath` that always
// reports "unavailable" so callers fall back to the `manually::*` userspace
// implementations, which use the allowed `openat` syscall. This mirrors the
// Android behavior, where the seccomp policy also prevents `openat2`.

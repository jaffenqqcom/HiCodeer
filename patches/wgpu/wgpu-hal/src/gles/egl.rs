...
mod ohos_native {
    ...
    pub const SET_BUFFER_GEOMETRY: u32 = 0;
    pub const SET_FORMAT: u32 = 3;

    pub fn set_buffer_geometry(
        window: *mut c_void,
        width: c_int,
        height: c_int,
    ) -> Result<(), c_int> {
        let ret = unsafe {
            super::OH_NativeWindow_NativeWindowHandleOpt(
                window,
                SET_BUFFER_GEOMETRY,
                width,
                height,
    ...
    pub fn set_format(window: *mut c_void, format: c_int) -> Result<(), c_int> {
        let ret = unsafe {
            super::OH_NativeWindow_NativeWindowHandleOpt(
                window,
                SET_FORMAT,
...
impl crate::Instance for Instance {
    ...
    unsafe fn init(desc: &crate::InstanceDescriptor<'_>) -> Result<Self, crate::InstanceError> {
        ...
        let egl_result = if cfg!(windows) {
            ...
            // 先用默认加载 libEGL.so.1（标准 EGL 实现）。避免加载华为
            // EglWrapper（libEGL.so），该封装层存在渲染 bug：clear 可
            // 工作但 draw call 不输出像素。若默认加载失败再试 libEGL.so。
            ...
            (Rwh::OhosNdk(handle), _) => {
                let fmt = inner
                    ...
                    .map_err(instance_err(
                        "failed to get config NATIVE_VISUAL_ID",
                    ))?;

                // 注意：不在 create_surface 中设置 SET_BUFFER_GEOMETRY（尺寸）。
                // 缓冲区几何尺寸由 ohos_window_manager.rs 在初始化/resize 时通过
                // OH_NativeWindow_NativeWindowHandleOpt 统一管理。
                // 此处只设像素格式，确保 EGL surface 的格式与本地窗口匹配。
                if let Err(ret) = ohos_native::set_format(
                    handle.native_window.as_ptr(),
                    fmt,
                ) {
                    return Err(crate::InstanceError::new(format!(
                        "OH_NativeWindow_NativeWindowHandleOpt(SET_FORMAT) \
                         returned {ret}",
                    )));
                }
            }

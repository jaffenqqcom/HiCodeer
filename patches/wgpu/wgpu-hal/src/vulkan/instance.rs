...
impl super::Instance {
    ...
    pub fn desired_extensions(
        ...
        if cfg!(target_env = "ohos") {
            // VK_OHOS_surface — OHOS Vulkan surface extension
            static VK_OHOS_SURFACE_NAME: &[u8] = b"VK_OHOS_surface ";
            extensions.push(unsafe {
                std::ffi::CStr::from_bytes_with_nul_unchecked(VK_OHOS_SURFACE_NAME)
            });
        }
    ...
    fn create_surface_ohos(
        ...
        let ext_name = std::ffi::CStr::from_bytes_with_nul(b"VK_OHOS_surface\0").unwrap();
        if !self.shared.extensions.iter().any(|e| (*e).to_bytes() == ext_name.to_bytes()) {
            return Err(crate::InstanceError::new(String::from(
                "Vulkan driver does not support VK_OHOS_surface",
            )));
        }

        // load vkCreateSurfaceOHOS via vkGetInstanceProcAddr
        let func_name = std::ffi::CString::new("vkCreateSurfaceOHOS").unwrap();
        let func_ptr = unsafe {
            self.shared.entry.get_instance_proc_addr(
        ...
        if func_ptr.is_none() {
            return Err(crate::InstanceError::new(String::from(
                "failed to load vkCreateSurfaceOHOS",
            )));
        }

        // Define function pointer type and struct locally (not in ash)
        type VkSurfaceCreateFunc = unsafe extern "system" fn(
            vk::Instance,
        ...
        let vk_create_surface: VkSurfaceCreateFunc = unsafe {
            std::mem::transmute(func_ptr.unwrap())
        };

        // VK_STRUCTURE_TYPE_SURFACE_CREATE_INFO_OHOS from the OHOS Vulkan header
        ...
        let create_info = VkSurfaceCreateInfoOHOS {
        ...
        let mut surface_handle = vk::SurfaceKHR::null();
        let result = unsafe {
            vk_create_surface(
                self.shared.raw.handle(),
                &create_info as *const _ as *const std::ffi::c_void,
                std::ptr::null(),
                &mut surface_handle,
            )
        };
        ...
        Ok(self.create_surface_from_vk_surface_khr(surface_handle))
    }
        ...
        match (window_handle, display_handle) {
            ...
            (Rwh::OhosNdk(_), _) => {
                Err(crate::InstanceError::new(String::from(
                    "OHOS surface is not supported on this platform",
                )))
            }
            (Rwh::Win32(handle), _) => {

...
pub fn lazy_per_thread_init() {
    ...
    unsafe fn allocate_sigaltstack() -> Option<Stack> {
        ...
        // On OHOS the sandbox rejects an `mprotect` that turns PROT_NONE pages
        // into READ|WRITE (returns ENOMEM), so allocate the stack region RW
        // from the start and skip the mprotect below.
        #[cfg(target_env = "ohos")]
        ...
        // On OHOS the region is already READ|WRITE from mmap (see above), so
        // no mprotect is needed there; the PROT_NONE -> RW mprotect is rejected
        // by the sandbox with ENOMEM.
        #[cfg(not(target_env = "ohos"))]
        unsafe {
            rustix::mm::mprotect(
                stack_ptr,
    ...
    impl Drop for Stack {
        fn drop(&mut self) {
            unsafe {
                // Disable the altstack registration before unmapping so the
                // kernel no longer references the memory: an SA_ONSTACK signal
                // delivered between here and thread exit would otherwise write
                // a frame into freed memory (see wasmtime#13857).
